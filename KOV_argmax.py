...
class BlackBoxMDP:
    ...
    def actions(self, state: BlackBoxState):
        if self.flipped:
            ...
            suffix = evaluate_model(self.target_model, prompt, temperature=0)
...
def transfer(prompt, suffixes, target_model, temperature=0, verbose=True, **kwargs):
    ...
    for i, suffix in enumerate(suffixes):
        ...
        r_mean, r_max = score_ALL(response, verbose=verbose, **kwargs)
        scores_mean.append(r_mean)
        scores_max.append(r_max)
        scores = [scores_mean, scores_max]
        ...
        if verbose:
            ...
            # print(f"[SCORE]: {r}\n")
            print(f"[SCORE_mean]: {r_mean}\n")
            print(f"[SCORE_max]: {r_max}\n")
            print("-" * 50)

        mod = moderation(response)
        mods.append(mod)
    return responses, scores, mods
    # return responses, scores_mean, scores_max, mods
...
def generate_training_data(benchmark_idx, csvFileName, model_name):
    ...
    print(f"Generating_Prompt:{prompt}")
    print(f"Generating_Target:{target}")
...
def score_ALL(response, use_perspective=False, verbose=False, use_mean=True, return_all=False, **kwargs):
    try:
        if use_perspective:
            # Placeholder for Perspective API scoring
            scores = {}  # Implement Perspective API call here if available
            ...
            if use_mean:
                key_mean = "Mean"
                r_mean = np.mean(list(scores.values()))
                # max
                key_max, r_max = max(scores.items(), key=lambda item: item[1])
            header = "Moderation Key"

        if verbose:
            print(f"\n{header}: {key_mean}\nScore: {r_mean}\n")
            print_box([[key_mean, r_mean]], [header, "Score"], color="bold yellow")
            # max
            print(f"\n{header}: {key_max}\nScore: {r_max}\n")
            print_box([[key_max, r_max]], [header, "Score"], color="bold yellow")

        return scores if return_all else r_mean, r_max
...
def startKOV1(csvFileName, model_name):
    ...
    results = test_transfer(fn_training_data, target_model, csvFileName, model_name, benchmark_indices=[88], temperature=0, verbose=True)
    # Print results
