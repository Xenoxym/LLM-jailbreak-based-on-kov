...
class WhiteBoxMDP:
    ...
    def optimize(self, state=None, show_response=True, return_logs=False):
        ...
        for step in range(params.num_steps):
            ...
            # Plotting the loss
            plot_loss(losses, filename=f"losses_{params.benchmark_idx}.png",flipped=params.flipped)
    ...
    def loss(self, state):
        ...
        target_slice = state.suffix_manager._target_slice  # The slice indicating the target portion
        target_start = target_slice.start
        target_end = target_slice.stop
        ...
        with torch.no_grad():
            ...
            shift_labels = input_ids[:, target_start:target_end].contiguous()
    ...
    def actions(self, state: WhiteBoxState):
        ...
        suffix_manager = state.suffix_manager  # 获取 suffix_manager
        
        # 获取 suffix 的切片范围
        suffix_slice = suffix_manager._control_slice
        suffix_start = suffix_slice.start
        suffix_end = suffix_slice.stop
        ...
        with torch.no_grad():
            ...
            # Get logits for the last token OF "SUFFIX"
            logits = logits.squeeze(0)  
            next_token_logits = logits[suffix_end - 1, :] 
    ...
    def compute_loss(self, state: WhiteBoxState):
        ...
        # 从 state 中获取 input_ids 和 suffix_manager
        input_ids = state.input_ids.to(params.device)  # Shape: [batch_size, seq_length]
        suffix_manager = state.suffix_manager  # 获取 suffix_manager
        ...
        with torch.no_grad():            
            ...
            dimensions1 = shift_logits.size()  # 或 target_ids.shape
            print(f"Dimensions of shift_logits: {dimensions1}")
            dimensions2 = shift_labels.size()  # 或 target_ids.shape
            print(f"Dimensions of shift_labels: {dimensions2}")

            ins = suffix_manager.target
            print(f"targrt to train : {ins}")
            ...
            nll = loss * (target_length - 1)
            state.nll = nll
            log_ppl = nll / (target_length - 1)
